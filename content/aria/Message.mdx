...
The `Message` component lays out a single message row. It handles the avatar, alignment, header, and footer around the visible message surface. Render the surface with [`Bubble`](/components/Bubble), and wrap long transcripts with [`MessageScroller`](/components/MessageScroller).
