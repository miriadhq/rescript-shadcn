...
let make = (props: ReactAria.RadioGroup.props) => {
  let dataSlot = props.dataSlot->Option.getOr("radio-group")
  <ReactAria.RadioGroup
    {...props} dataSlot className={cn("cn-radio-group w-full", props.className)}
  />
}
module Item = {
  ...
  let make = (props: props<'children>) => {
    let dataSlot = props.dataSlot->Option.getOr("radio-group-item")
    <ReactAria.Radio
