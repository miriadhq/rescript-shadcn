...
let make = ({}: Demo.Props.t) => {
  ...
  <Dialog.Trigger isOpen={open_} onOpenChange={value => setOpen(_ => value)}>
    <Button variant=Outline> {React.string("Open clarification")} </Button>
    <Dialog>
      <Questionnaire defaultItem="scope" items={items} onSubmit={handleSubmit}>
        ...
        <Dialog.Footer>
          <Dialog.Close type_="button" variant=Outline> {React.string("Cancel")} </Dialog.Close>
