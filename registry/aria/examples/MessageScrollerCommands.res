...
module CommandMenu = {
  ...
  let make = () => {
    ...
    <DropdownMenu.Trigger>
      ...
      <DropdownMenu placement=ReactAria.Common.BottomEnd className="w-64">
        ...
        ->Array.map(message =>
          <DropdownMenu.Item
            ...
            onAction={() =>
              scrollToMessage(
                message.id,
                Some({align: Start, behavior: Smooth}),
              )->ignore}
          >
    ...
    <div className="relative flex flex-col gap-4">
      <Card className="mx-auto h-140 w-full max-w-sm gap-0">
        <Card.Header className="gap-1 border-b">
          ...
          <Card.Description> {"Drive the transcript from outside."->React.string} </Card.Description>
          <Card.Action> <CommandMenu /> </Card.Action>
        </Card.Header>
