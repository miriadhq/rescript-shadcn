    ...
    <Card className="mx-auto h-140 w-full max-w-sm gap-0">
      ...
      <Card.Footer>
        <ToggleGroup
          ...
          multiple=false
          value={[(anchorRole :> string)]}
          onValueChange={(keys, _) =>
        ...
        >
          <ToggleGroup.Item value="user" ariaLabel="Anchor user messages">
            {"User"->React.string}
          </ToggleGroup.Item>
          <ToggleGroup.Item value="assistant" ariaLabel="Anchor assistant messages">
        ...
        <Button
          ...
          size=Icon
          className="ml-auto"
          disabled={nextMessage->Option.isNone}
          onClick={_ => nextMessage->Option.forEach(_ => setMessageIndex(index => index + 1))}
        >
          <Icons.ArrowUp />
