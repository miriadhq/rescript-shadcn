...
let make = (~className=?, ~children=?) => {
  ...
  >
    ...
    <Collapsible.Trigger className="from-code/70 to-code text-muted-foreground absolute inset-x-0 -bottom-2 flex h-20 items-center justify-center rounded-b-lg bg-gradient-to-b text-sm group-data-[state=open]/collapsible:hidden">
      {(isOpened ? "Collapse" : "Expand")->React.string}
    </Collapsible.Trigger>
  </Collapsible>
}
